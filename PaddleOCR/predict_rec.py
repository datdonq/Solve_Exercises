...
class TextRecognizer(object):
    def __init__(self, use_gpu,dictionary,rec_model_dir):
        ...
        self.predictor, self.input_tensor, self.output_tensors, self.config = \
            utility.create_predictor(rec_model_dir,use_gpu)
...
def run_text_rec(img_list, use_gpu,dictionary,rec_model_dir):
    # args = utility.parse_args()
    # args.use_gpu = use_gpu
    # # args.det_algorithm = det_algorithm
    # args.rec_model_dir = rec_model_dir
    # args.rec_char_dict_path=dictionary
    text_recognizer = TextRecognizer(use_gpu,dictionary,rec_model_dir)
...
#                                                rec_res[ino]))
#         logger.info("The predict time is:{}".format(elapse))
        
#         tup+=((valid_image_file_list[ino],rec_res[ino][0]),)
#     def extract_number(filename):
#       return int(filename.split("/")[-1].split(".")[0])
#     sorted_tuple = sorted(tup, key=lambda item: extract_number(item[0]))
#     print(sorted_tuple)
#     with open(text_recognizer.save_rec_path, 'w') as file:
#         for item in sorted_tuple:
#         # Ghi phần tử thứ hai của mỗi phần tử trong tuple vào file
#           file.write(item[1] + " ")
#     if args.benchmark:
