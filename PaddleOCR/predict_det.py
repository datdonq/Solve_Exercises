...
class TextDetector(object):
    def __init__(self, use_gpu, det_model_dir):
        # self.args = args
        self.det_algorithm = det_model_dir
        self.use_onnx = False
        pre_process_list = [{
            'DetResizeForTest': {
                'limit_side_len': 960,
        ...
        self.predictor, self.input_tensor, self.output_tensors, self.config = utility.create_predictor(self.det_algorithm,use_gpu)
...
logger = get_logger()
def run_text_detector(img, use_gpu, det_model_dir):
    s_img=img
    ...
    text_detector = TextDetector(use_gpu,det_model_dir)
    total_time = 0
